...
class KDD99Loader(object):
    def __init__(self, data_path, mode="train"):
        ...
        N_test = N_normal -N_train
        self.train = normal_data[:N_train]
        self.train_labels =np.zeros(self.train.shape[0])

        self.test = normal_data[N_train:]
        self.test_labels = np.zeros(self.test.shape[0])

        self.test = np.concatenate((self.test, attack_data[:N_test]),axis=0)
        self.test_labels = np.concatenate((self.test_labels, np.ones(N_test)),axis=0)
        pass
